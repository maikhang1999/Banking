...
// TransferTx performs a mony transfer from one account to the other
// It creates a transfer record, add account entries, and update accounts balance within a single db tranction
func (SQLStore *SQLStore) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := SQLStore.execTx(ctx, func(q *Queries) error {
		var err error
		result.Transfer, err = SQLStore.CreateTransfer(ctx, CreateTransferParams{
